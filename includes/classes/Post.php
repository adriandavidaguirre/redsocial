...
class Post {
  ...
  (id, body, added_by, user_to, date_added, user_closed, deleted, likes, image)
  VALUES (
    '', '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0', '$imageName'
  )");
/*$query = mysqli_query($this->con, "INSERT INTO posts 
  (id, body, added_by, user_to, date_added, user_closed, deleted, likes)
  VALUES (
    ...
    $term = ucfirst($value);
    $this->calculateTrend($term);

    // Inserta la relación en trends_posts
    $insertTrendPost = mysqli_query($this->con, "INSERT INTO trends_posts (trend_title, post_id) VALUES ('$term', '$returned_id')");
}
 ...
 $first_name $last_name </a> $user_to &nbsp;&nbsp;&nbsp;&nbsp;$time_message
									$delete_button
								</div>
								<div id='post_body'>
									$body
    ...
    $('#post<?php echo $id; ?>').off().on('click', function() {
					/*$(document).ready(function() {
			...
			if($count > $limit) 
				...
				$str .= "<input type='hidden' class='noMorePosts' value='true'><p style='text-align: centre;' class='noMorePostsText'> No mas post! </p>";
		}
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
					...
					$str .= "<div class='status_post' onClick='javascript:toggle$id()'>
								...
								<div class='posted_by' style='color:#ACACAC;'>
									<a href='profile.php?profile_username=$added_by'>
 $first_name $last_name </a> &nbsp;&nbsp;&nbsp;&nbsp;$time_message
									$delete_button
								</div>
								<div id='post_body'>
									$body
